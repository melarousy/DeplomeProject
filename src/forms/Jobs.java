...
public class Jobs extends javax.swing.JFrame {
    ...
    String Query = "SELECT ID as [الكود] ,JOB_NAME as [الوظيفة] FROM JOBS WHERE I=0";
    int ClincId = 0;

    /**
     * Creates new form Clinics
     */
    private static Jobs obj=null;
    private Jobs() {
        initComponents();
        this.setTitle("الوظائف");
    ...
    private void initComponents() {
        ...
        jLabel2.setText("لتعديل او حذف وظيفة قم باختيارها من الجدول");
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        if (jTextField1.getText().equals("")) {
            JOptionPane.showMessageDialog(this, "ادخل اسم الوظيفة", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
            Save_Data();
        }
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if (ClincId > 0 && ClincId < 4) {
            jTextField1.setText("");
            JOptionPane.showMessageDialog(this, "لايمكن تعديل او حذف الوظائف الاساسية", "معلومة", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
    ...
    public void Save_Data() {
        try {
            String sql1 = "Select ID,JOB_NAME ,I from JOBS where JOB_NAME= '" + jTextField1.getText() + "'";

            ResultSet rs = Data.Get_Data(sql1);

            if (rs.next()) {
                if (rs.getInt("I") == 0) {
                    JOptionPane.showMessageDialog(this, "الوظيفة موجودة من قبل", "خطأ", JOptionPane.ERROR_MESSAGE);
                    jTextField1.setText("");
                    jTextField1.requestDefaultFocus();
                } else {
                    try {
                        int P = JOptionPane.showConfirmDialog(this, "الوظيفة محذوفة هل تريد استعاداتها", "تأكيد", JOptionPane.YES_NO_OPTION);
                        if (P == 0) {
                            String sql = "update JOBS set I='" + 0 + "' where ID='" + rs.getInt("ID") + "'";
                            int i = Data.Save_Data(sql);
                            if (i == 0) {
                                JOptionPane.showMessageDialog(this, "تم استعادة الوظيفة", "استعادة وظيفة", JOptionPane.INFORMATION_MESSAGE);
                ...
                String sql = "insert into JOBS (JOB_NAME,I) values ('" + jTextField1.getText() + "','" + 0 + "')";
                int i = Data.Save_Data(sql);
                if (i == 0) {
                    JOptionPane.showMessageDialog(this, "تم الحفظ بنجاح", "اضافة وظيفة", JOptionPane.INFORMATION_MESSAGE);
                    refrishtable();
                    btnSave.setEnabled(false);
                }
        ...
        try {
            ...
            if (rs.next()) {
                if (rs.getInt("I") == 0) {
                    ...
                    int t = JOptionPane.showConfirmDialog(this, "الوظيفة محذوفة هل تريد استعاداتها", "تأكيد", JOptionPane.YES_NO_OPTION);
                    if (t == 0) {
                        String sql = "update JOBS set I='" + 0 + "' where ID='" + rs.getInt("ID") + "'";
                        int i = Data.Save_Data(sql);
                        if (i == 0) {
                ...
                int P = JOptionPane.showConfirmDialog(this, "هل تريد تعديل الوظيفة", "تأكيد", JOptionPane.YES_NO_OPTION);
                if (P == 0) {
                    String sql = "update JOBS set JOB_NAME='" + jTextField1.getText() + "' where ID='" + ClincId + "'";
                    int i = Data.Save_Data(sql);
                    if (i == 0) {
                        JOptionPane.showMessageDialog(this, "تم التعديل بنجاح", "تعديل وظيفة", JOptionPane.INFORMATION_MESSAGE);
                        reset();
                    }
                }
            }
    ...
    public void Delete_Data() {
        try {
            int P = JOptionPane.showConfirmDialog(this, "سيتم حذف الوظيفة ولن تتمكن من استعادتها", "تأكيد", JOptionPane.YES_NO_OPTION);
            if (P == 0) {
                Connection con = DatabaseConnection.con();
                PreparedStatement pst = null;

                String sql = "update JOBS set I='" + 1 + "' where ID='" + ClincId + "'";

                pst = con.prepareStatement(sql);
                pst.execute();
                JOptionPane.showMessageDialog(this, "تم الحذف بنجاح", "حذف وظيفة", JOptionPane.INFORMATION_MESSAGE);
                btnSave.setEnabled(true);
                btnedite.setEnabled(false);
                btndelete.setEnabled(false);
                refrishtable();
